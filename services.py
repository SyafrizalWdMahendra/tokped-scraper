...
#     model_db = await prisma.model.find_first(where={"modelName": "Model XGBoost (Optimized)"})
#     if not model_db:
...
#             print(f"🛑 ABSA Dibatalkan di tengah jalan untuk {candidate.name}")
#             return None
...
async def process_product_reviews(candidate: ProductCandidate, user_email: str, metric_id: int, brand_id: int, request: Request):
    user_db = await prisma.user.find_unique(where={"email": user_email})
    if not user_db:
        print(f"⚠️ User tidak ditemukan untuk email: {user_email}")
        raise ValueError(f"User dengan email {user_email} tidak ditemukan di database")
    ...
    model_db = await prisma.model.find_first(where={"modelName": "Model XGBoost (Optimized)"})
    if not model_db:
        print("❌ ERROR: Model XGBoost (Optimized) tidak ditemukan di database!")
        return None
